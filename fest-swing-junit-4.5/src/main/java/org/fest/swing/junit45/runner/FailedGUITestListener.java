...
/**
 * Understands a JUnit 4.5 <code>{@link RunListener}</code> that takes a screenshot of failed GUI tests.
 *
 * @author Alex Ruiz
 * @author Yvonne Wang
 */
public class FailedGUITestListener extends RunListener {
  ...
  private void takeScreenshot(Failure failure) {
    ...
    Description description = descriptionForGUITest(failure);
    if (description == null) return;
    String methodSignature = description.getDisplayName();
    try {
  ...
  private Description descriptionForGUITest(Failure failure) {
    Description description = failure.getDescription();
    GUITest annotation = description.getAnnotation(GUITest.class);
    return annotation != null ? description : null;
  }
}
