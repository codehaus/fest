 ...
 * @author Yvonne Wang
 */
public class DialogLauncherWindow extends TestWindow {
  ...
  @RunsInEDT
  public static DialogLauncherWindow createNew(final Class<?> testClass) {
    return execute(new GuiQuery<DialogLauncherWindow>() {
      protected DialogLauncherWindow executeInEDT() {
        return new DialogLauncherWindow(testClass);
      }
    });
  }

  private DialogLauncherWindow(Class<?> testClass) {
    super(testClass);
